...
class NBBCJRFormerMarkerTrainer(BaseTrainer[BCJRFormerMarkerDataset]):
    def __init__(
        self,
        ...
        if q <= 2:
            raise ValueError("q must be greater than 2 for nonbinary BCJRFormer")
    ...
    def epoch_train(
        ...
        for batch_idx, (
            ...
            loss = F.cross_entropy(x_pred.permute(0, 2, 1), x_i.long().to(self.device))
